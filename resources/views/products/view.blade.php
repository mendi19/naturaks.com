		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb py-2 py-lg-3">
					...
					<a href="{{route('shop.index')}}">{{trans('global.home')}}</a>
								...
								<li class="breadcrumb-item">
					<a href="{{route('shop.products')}}">{{trans('global.products')}}</a>
				</li>
		...
		<div class="container">
			...
			<div class="row justify-content-lg-between">
				...
				<div class="col-12">
					...
					<input type="hidden" value="{{$data->id}}-0" id="ID_CURRENT" name="ID_CURRENT">
				</div>
						...
						@if(isset($data->getimages) && $data->getimages->count() > 0)
							...
							<div>
								<div class="img-wrapper position-relative bg-light">
									...
									<img src="{{asset('images/icons/loading.gif')}}" data-src="{{featured_image_v3($vv->package_id,$vv->image,'xs')}}" class="w-100 lazy">
								</div>
							</div>
							@endforeach
						@endif
				...
				<div class="col-md-6">
					...
					<div >
						<input type="text" name="product" value="NT0024" hidden="">
						<div class="row">
							<div class="col-12">
								@if(getfinalprice($data->price,$data->discounted_price,$data->discount_status,$data->discount_from_date,$data->discount_to_date,$data->discount_permanent_yesno,$data->discount_kolicina,$data->discount_kolicina_nolimit,trans('general.currency')) != $data->price)

								<p class="h6 product-title-2 text-sm font-weight-bold mb-2"><span class="text-del">{{$data->price}} {{trans('general.currency')}}.</span></p>
							@endif

																<p class="h2 text-red mb-3 mb-lg-5">{{displayfinalprice($data->price,$data->discounted_price,$data->discount_status,$data->discount_from_date,$data->discount_to_date,$data->discount_permanent_yesno,$data->discount_kolicina,$data->discount_kolicina_nolimit,trans('general.currency'))}}.</p>
							</div>
							<div class="order-controls-wrapper form-group col-auto mb-0">
								<input type="number" min="1" max="999" value="1" name="qty" class="form-control h-100 increment-control quantity_0_{{$data->id}}" id="quantity">
								<div class="wrapper pl-1">
									<button type="button" class="controls control-increase" data-id="{{$data->id}}" data-type="0">+</button>
									<button type="button" class="controls control-decrease" data-id="{{$data->id}}" 
								   data-type="0">-</button>
								</div>
							</div>
...
<div class="product-col @if($vv->package->status_webpage == 0) d-none @endif">
