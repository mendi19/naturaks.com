@section('meta_og_title',$meta_title)
...
@section('content')
	...
	<div class="banner banner-sm bg-image" style="background-image: url({{asset('assets/images/banners/top-banner.jpg')}})">
			...
			<div class="container">
				<h2 class="h1">{{trans('global.products')}}</h2>
			</div>
		</div>
			</div>
</section>
...
<div class="row products-row mb-4">
	<div class="col-md-12">
		<ul class="listcategories">
			<li class="text-uppercase"><a href="{{route('shop.products')}}" @if($category_id == 0) class="active_category" @endif>
			{{trans('global.allproducts')}}</a></li>

			@foreach(trans('arrays.products_categories') as $k=> $v)
				@if($k>0)
				<li ><a href="{{route('product.by.category',array('slug'=> trans('arrays.products_categories_slugs')[$k]))}}" @if($category_id == $k) class="active_category" @endif>{{$v}}</a></li>
				@endif
			@endforeach
		</ul>
	</div>
			...
			<a href="{{route('shop.product.view',array('slug' => urlslugcheck($value->slug,$value->id)))}}" class="link link-dark">
					<div class="animation-content mb-3">
						...
						@if(getfinalprice($value->price,$value->discounted_price,$value->discount_status,$value->discount_from_date,$value->discount_to_date,$value->discount_permanent_yesno,$value->discount_kolicina,$value->discount_kolicina_nolimit,trans('general.currency')) != $value->price)
								...
								<p class="h6 product-title-2 text-sm font-weight-bold mb-2"><span class="text-del">{{$value->price}} {{trans('general.currency')}}.</span></p>
							@endif
													<p class="h6 product-title-2 text-sm text-red font-weight-bold mb-0">  {{displayfinalprice($value->price,$value->discounted_price,$value->discount_status,$value->discount_from_date,$value->discount_to_date,$value->discount_permanent_yesno,$value->discount_kolicina,$value->discount_kolicina_nolimit,trans('general.currency'))}}.</p>
						</div>
					</div>				
				</a>
